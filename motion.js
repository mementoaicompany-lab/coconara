...
function setupFerryStatus(){
 ...
 const scene=document.createElement('div');scene.className='ferry-sailing-scene';scene.dataset.localized='true';scene.innerHTML='<div class="sailing-speech"></div><img src="sailing-ferry.webp" class="sailing-vessel" alt=""><button type="button"></button>';banner.firstElementChild.after(scene);
 const badge=$('#fsb-badge');badge.dataset.localized='true';badge.setAttribute('aria-live','polite');let paused=matchMedia('(prefers-reduced-motion: reduce)').matches,visible=true;
 window.CocoSmoothMotion.ocean(scene);
 matchMedia('(prefers-reduced-motion: reduce)').addEventListener('change',e=>{if(e.matches){paused=true;sync();}});
 function sync(){scene.querySelector('.sailing-speech').textContent=window.COCO_CHOICE_COPY?.[window.cocoLanguage?.()||'ko']?.ferryBubble||'코코나라는 하우목동항!';if(window.CoconaraFerry) {const state=window.CoconaraFerry.getState().status;scene.hidden=state!=='normal';scene.querySelector('.sailing-vessel').alt=words().shipAlt;scene.style.setProperty('--scene-motion',paused||!visible||document.hidden||state!=='normal'?'paused':'running');const button=scene.querySelector('button');button.textContent=paused?'▶':'⏸';button.setAttribute('aria-label',words()[paused?'play':'pause']);button.setAttribute('aria-pressed',String(paused));return;}const state=['normal','cancel','pending','closed','shortened'].find(s=>banner.classList.contains(s))||'pending',data=window.FERRY_STATUS_MAP[state];badge.hidden=state==='normal';badge.textContent=tr(data.badge||data.title);badge.style.background=data.badgeBg||({pending:'#aa750f',closed:'#526078'}[state]);badge.style.color='#fff';scene.hidden=state!=='normal';scene.querySelector('.sailing-vessel').alt=words().shipAlt;scene.style.setProperty('--scene-motion',paused||!visible||document.hidden||state!=='normal'?'paused':'running');const button=scene.querySelector('button');button.textContent=paused?'▶':'⏸';button.setAttribute('aria-label',words()[paused?'play':'pause']);button.setAttribute('aria-pressed',String(paused));}
...
function setupBroadcastAutoplay() {
  const frames = $$('#page-broadcast iframe[src*="youtube.com/embed/"],#page-coconara iframe[src*="youtube.com/embed/"],#page-dalkom iframe[src*="youtube.com/embed/"],#page-hundert iframe[src*="youtube.com/embed/"]');
