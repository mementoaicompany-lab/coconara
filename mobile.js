...
function setupNavigation(){
 ...
 window.showPage=function(id){original.call(this,id,buttons[order.indexOf(id)]);sync();};
...
function setupHarbour(){
 ...
 let elapsed=0,last=0,visible=true;
 if('IntersectionObserver'in window)new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;}).observe(map);
 function frame(now){const delta=last?Math.min(100,now-last):0;last=now;const paused=$('.map-motion')?.getAttribute('aria-pressed')==='true';if(!paused&&visible&&!document.hidden&&!reduced.matches)elapsed+=delta;
 ...
 const x=-13+80*progress,y=185+6*progress*(1-progress)+(paused||reduced.matches?0:Math.sin(elapsed/380)*.6);
 ...
 }boat.setAttribute('transform','translate(30,187) scale(.62) translate(-29,-158)');requestAnimationFrame(frame);
}
...
function setupDalkomGallery(){
 ...
 const stage=gallery.querySelector('.dalkom-gallery-stage'),thumbs=gallery.querySelector('.dalkom-gallery-thumbs');let current=0,paused=reduced.matches,visible=false;
 ...
 function openFull(index){if(stage.dataset.swiped)return;paused=true;refresh();const overlay=document.createElement('div');overlay.className='dalkom-fullscreen';overlay.dataset.localized='true';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.setAttribute('aria-label',t('📸 달콤아재 사진'));
 ...
 if('IntersectionObserver'in window)new IntersectionObserver(entries=>visible=entries[0].isIntersecting,{threshold:.25}).observe(gallery);else visible=true;
