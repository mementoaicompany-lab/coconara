...
function start(){setupMap();setupSpots();const close=document.createElement('button');close.className='spot-close-button';close.type='button';close.textContent='×';close.setAttribute('aria-label',tr('닫기'));close.addEventListener('click',()=>window.closeSpot());$('#spotPopup').prepend(close);$('#spotPopup').setAttribute('role','dialog');$('#spotPopup').setAttribute('aria-modal','true');$('#spotPopup').setAttribute('aria-labelledby','spotTitle');localize();updateTimeLabels();$$('a[target="_blank"]').forEach(a=>a.rel='noopener noreferrer');const hash=location.hash.slice(1);if(['home','ferry','coconara','broadcast','dalkom','hundert','food'].includes(hash))window.showPage(hash,$$('.tab-item').find(tab=>tab.dataset.page===hash));}
