...
function setupVehicles(){
 $$('.vehicle-stage').forEach((stage,index)=>{
  ...
  if('IntersectionObserver'in window)new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;refresh();},{threshold:.15}).observe(stage);
...
function setupOwner(){
 ...
 const titles=$$('.owner-reading .reading-title');let paused=reduced.matches,visible=!('IntersectionObserver'in window);
 function refresh(){
 ...
 if('IntersectionObserver'in window)new IntersectionObserver(entries=>{visible=entries[0].isIntersecting;refresh();},{threshold:.15}).observe(header);
 document.addEventListener('visibilitychange',refresh);refreshers.push(refresh);refresh();
}
